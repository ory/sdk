...
pub async fn admin_get_jwks(configuration: &configuration::Configuration, ) -> Result<models::GetJwksResponse, Error<AdminGetJwksError>> {

    let uri_str = format!("{}/v2alpha1/admin/derivedKeys/jwks.json", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<AdminGetJwksError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

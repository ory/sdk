...
/// struct for typed errors of method [`delete_rotated_o_auth2_client_secrets`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum DeleteRotatedOAuth2ClientSecretsError {
...
/// struct for typed errors of method [`rotate_o_auth2_client_secret`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub async fn delete_rotated_o_auth2_client_secrets(configuration: &configuration::Configuration, id: &str) -> Result<models::OAuth2Client, Error<DeleteRotatedOAuth2ClientSecretsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;

    let uri_str = format!("{}/admin/clients/{id}/secrets/rotate", configuration.base_path, id=crate::apis::urlencode(p_path_id));
    let mut req_builder = configuration.client.request(reqwest::Method::DELETE, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<DeleteRotatedOAuth2ClientSecretsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn rotate_o_auth2_client_secret(configuration: &configuration::Configuration, id: &str, rotate_o_auth2_client_secret_body: Option<models::RotateOAuth2ClientSecretBody>) -> Result<models::OAuth2Client, Error<RotateOAuth2ClientSecretError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
    let p_body_rotate_o_auth2_client_secret_body = rotate_o_auth2_client_secret_body;
    ...
    req_builder = req_builder.json(&p_body_rotate_o_auth2_client_secret_body);

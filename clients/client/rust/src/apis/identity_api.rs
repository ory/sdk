...
pub async fn list_identities(configuration: &configuration::Configuration, per_page: Option<i64>, page: Option<i64>, page_size: Option<i64>, page_token: Option<&str>, consistency: Option<&str>, ids_filter: Option<Vec<String>>, credentials_identifier: Option<&str>, preview_credentials_identifier_similar: Option<&str>) -> Result<Vec<crate::models::Identity>, Error<ListIdentitiesError>> {
    ...
    if let Some(ref local_var_str) = ids_filter {
        local_var_req_builder = local_var_req_builder.query(&[("ids_filter", local_var_str.iter().map(|p| p.to_string()).collect::<Vec<String>>().join(","))]);
    }

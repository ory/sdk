...
> models::RecoveryFlow create_browser_recovery_flow(return_to, skip_settings)
...
------------- | ------------- | ------------- | ------------- | -------------
**return_to** | Option<**String**> | The URL to return the browser to after the flow was completed. |  |
**skip_settings** | Option<**String**> | Skip redirection to the settings UI after the recovery flow was completed. Instead, the user will be redirected to the URL specified in `return_to` query parameter or the default return URL if `return_to` is not set. |  |
...
> models::ErrorGeneric create_id_p_initiated_saml_browser_login_flow(provider, code)
...
async fn main() {
    let mut configuration = Configuration::new();
    let provider = "provider_example"; // String | The SAML provider ID as configured in the Ory Kratos configuration.
    let code = "code_example"; // String | The single-use authorization code issued by Ory Polis for the unsolicited SAML response.
    match frontend_api::create_id_p_initiated_saml_browser_login_flow(&configuration, provider, code).await {
        Ok(response) => println!("FrontendApi::create_id_p_initiated_saml_browser_login_flow: {:?}", response),
        Err(error) => eprintln!("Error calling FrontendApi::create_id_p_initiated_saml_browser_login_flow: {:?}", error),
    }
}
```
...
Name | Type | Description  | Required | Notes
------------- | ------------- | ------------- | ------------- | -------------
**provider** | **String** | The SAML provider ID as configured in the Ory Kratos configuration. | [required] |
**code** | **String** | The single-use authorization code issued by Ory Polis for the unsolicited SAML response. | [required] |
...
async fn main() {
    let mut configuration = Configuration::new();
    match frontend_api::get_saml_metadata(&configuration).await {
        Ok(response) => println!("FrontendApi::get_saml_metadata: {:?}", response),
        Err(error) => eprintln!("Error calling FrontendApi::get_saml_metadata: {:?}", error),
    }
}
```
...
async fn main() {
    let mut configuration = Configuration::new();
    let provider = "provider_example"; // String | The SAML connection ID to get metadata for.
    match frontend_api::get_saml_provider_metadata(&configuration, provider).await {
        Ok(response) => println!("FrontendApi::get_saml_provider_metadata: {:?}", response),
        Err(error) => eprintln!("Error calling FrontendApi::get_saml_provider_metadata: {:?}", error),
    }
}
```
...
Name | Type | Description  | Required | Notes
------------- | ------------- | ------------- | ------------- | -------------
**provider** | **String** | The SAML connection ID to get metadata for. | [required] |
...
> models::WebAuthnRelatedOrigins get_web_authn_related_origins()
...
async fn main() {
    let mut configuration = Configuration::new();
    match frontend_api::get_web_authn_related_origins(&configuration).await {
        Ok(response) => println!("FrontendApi::get_web_authn_related_origins: {:?}", response),
        Err(error) => eprintln!("Error calling FrontendApi::get_web_authn_related_origins: {:?}", error),
    }
}
```
...
[**models::WebAuthnRelatedOrigins**](webAuthnRelatedOrigins.md)
...
async fn main() {
    let mut configuration = Configuration::new();
    let provider = "provider_example"; // String | The SAML connection ID to start a native SP-initiated sign-in for.
    let flow = "flow_example"; // String | The Login, Registration, or Settings Flow ID this SAML sign-in continues.
    let purpose = "purpose_example"; // String | The kind of flow `flow` names: `login`, `registration`, or `settings-link` (a settings flow linking a new SAML credential). The flow must exist in the named kind, or the request is not found.
    match frontend_api::init_saml_login(&configuration, provider, flow, purpose).await {
        Ok(response) => println!("FrontendApi::init_saml_login: {:?}", response),
        Err(error) => eprintln!("Error calling FrontendApi::init_saml_login: {:?}", error),
    }
}
```
...
Name | Type | Description  | Required | Notes
------------- | ------------- | ------------- | ------------- | -------------
**provider** | **String** | The SAML connection ID to start a native SP-initiated sign-in for. | [required] |
**flow** | **String** | The Login, Registration, or Settings Flow ID this SAML sign-in continues. | [required] |
**purpose** | **String** | The kind of flow `flow` names: `login`, `registration`, or `settings-link` (a settings flow linking a new SAML credential). The flow must exist in the named kind, or the request is not found. | [required] |
...
async fn main() {
    ...
    let flow = "flow_example"; // String | The Login, Registration, or Settings Flow ID this SAML sign-in continues.
    let purpose = "purpose_example"; // String | The kind of flow `flow` names: `login`, `registration`, or `settings-link` (a settings flow linking a new SAML credential). The flow must exist in the named kind, or the request is not found.
    match frontend_api::init_saml_login_request(&configuration, provider, flow, purpose).await {
        Ok(response) => println!("FrontendApi::init_saml_login_request: {:?}", response),
        Err(error) => eprintln!("Error calling FrontendApi::init_saml_login_request: {:?}", error),
    }
}
```
...
> models::ErrorGeneric submit_saml_assertion(provider, relay_state, saml_response)
...
async fn main() {
    let mut configuration = Configuration::new();
    let provider = "provider_example"; // String | The SAML connection ID this assertion is delivered for.
    let relay_state = "relay_state_example"; // String | The opaque token that correlates this response with the AuthnRequest issued by `POST /self-service/methods/saml/init/{provider}`.  The PascalCase property name deliberately violates this API's snake_case convention: `RelayState` is the literal form-field name mandated by the SAML 2.0 HTTP-POST binding (OASIS SAML bindings spec), and every identity provider posts exactly this name. Do not rename it.
    let saml_response = "saml_response_example"; // String | The base64-encoded, XML-serialized samlp:Response the identity provider produced for the AuthnRequest issued by `POST /self-service/methods/saml/init/{provider}`.  The PascalCase property name deliberately violates this API's snake_case convention: `SAMLResponse` is the literal form-field name mandated by the SAML 2.0 HTTP-POST binding (OASIS SAML bindings spec), and every identity provider posts exactly this name. Do not rename it.
    match frontend_api::submit_saml_assertion(&configuration, provider, relay_state, saml_response).await {
        Ok(response) => println!("FrontendApi::submit_saml_assertion: {:?}", response),
        Err(error) => eprintln!("Error calling FrontendApi::submit_saml_assertion: {:?}", error),
    }
}
```
...
Name | Type | Description  | Required | Notes
------------- | ------------- | ------------- | ------------- | -------------
**provider** | **String** | The SAML connection ID this assertion is delivered for. | [required] |

...
**identity_schema** | Option<**String**> | IdentitySchema optionally holds the ID of the identity schema that is used for this flow. This value can be set by the user when creating the flow and should be retained when the flow is saved or converted to another flow. | [optional]
...
**state** | [**models::RegistrationFlowState**](registrationFlowState.md) |  | 

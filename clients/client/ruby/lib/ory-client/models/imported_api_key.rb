...
module OryClient
  ...
  class ImportedApiKey < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OryClient::ImportedApiKey` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OryClient::ImportedApiKey`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [actor_id, create_time, expire_time, ip_restriction, key_id, last_used_time, metadata, name, rate_limit_policy, revocation_description, revocation_reason, scopes, status, update_time, visibility].hash
    end

...
class NormalizedProjectRevisionThirdPartyProvider(BaseModel):
    ...
    """ # noqa: E501
    aal2_acr_values: Optional[List[StrictStr]] = Field(default=None, description="AAL2ACRValues lists upstream OIDC `acr` claim values that should elevate the resulting Kratos session to AAL2. Empty means the upstream `acr` claim is ignored when deciding session AAL.")
    aal2_amr_values: Optional[List[StrictStr]] = Field(default=None, description="AAL2AMRValues lists upstream OIDC `amr` claim values that should elevate the resulting Kratos session to AAL2 when any of them appears in the upstream `amr` array. Empty means the upstream `amr` claim is ignored when deciding session AAL.")
    account_linking_mode: Optional[StrictStr] = Field(default=None, description="AccountLinkingMode controls how account conflicts are resolved for this provider.  Possible values are `confirm_with_existing_credential` (default) and `automatic`. `automatic` silently links accounts when the provider verifies email ownership. Only supported for `apple` and `google` providers. automatic AccountLinkingModeAutomatic silently links accounts if the provider verifies email ownership. confirm_with_existing_credential AccountLinkingModeConfirmWithExistingCredential requires the user to confirm the account linking by providing an existing credential.")
    additional_id_token_audiences: Optional[List[StrictStr]] = Field(default=None, description="AdditionalIDTokenAudiences is a list of additional audiences allowed in the ID Token.  This is only relevant in OIDC flows that submit an IDToken instead of using the callback from the OIDC provider.")
    ...
    front_channel_logout: Optional[StrictBool] = Field(default=None, description="FrontChannelLogout enables OpenID Connect Front-Channel Logout for this provider: a login issues a companion cookie that lets the provider end the resulting Ory session from its own sign-out page.  Requires the provider to return the `sid` claim in the ID token.")
    ...
    pkce: Optional[StrictStr] = None
    project_revision_id: Optional[UUID] = Field(default=None, description="The Revision's ID this provider belongs to")
    ...
    scope: Optional[List[StrictStr]] = Field(default=None, description="Scope specifies optional requested permissions.")
    ...
    @field_validator('update_identity_on_login')
    def update_identity_on_login_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['never', 'automatic']):
            raise ValueError("must be one of enum values ('never', 'automatic')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "aal2_acr_values": obj.get("aal2_acr_values"),
            "aal2_amr_values": obj.get("aal2_amr_values"),
            "account_linking_mode": obj.get("account_linking_mode"),
            ...
            "front_channel_logout": obj.get("front_channel_logout"),
            "id": obj.get("id"),
            ...
            "update_identity_on_login": obj.get("update_identity_on_login"),

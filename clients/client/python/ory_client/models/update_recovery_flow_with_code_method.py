...
class UpdateRecoveryFlowWithCodeMethod(BaseModel):
    ...
    recovery_address: Optional[StrictStr] = Field(default=None, description="A recovery address that is registered for the user. It can be an email, a phone number (to receive the code via SMS), etc. Used in RecoveryV2.")
    recovery_confirm_address: Optional[StrictStr] = Field(default=None, description="If there are multiple recovery addresses registered for the user, and the initially provided address is different from the address chosen when the choice (of masked addresses) is presented, then we need to make sure that the user actually knows the full address to avoid information exfiltration, so we ask for the full address. Used in RecoveryV2.")
    recovery_select_address: Optional[StrictStr] = Field(default=None, description="If there are multiple addresses registered for the user, a choice is presented and this field stores the result of this choice. Addresses are 'masked' (never sent in full to the client and shown partially in the UI) since at this point in the recovery flow, the user has not yet proven that it knows the full address and we want to avoid information exfiltration. So for all intents and purposes, the value of this field should be treated as an opaque identifier. Used in RecoveryV2.")
    screen: Optional[StrictStr] = Field(default=None, description="Set to \"previous\" to go back in the flow, meaningfully. Used in RecoveryV2.")
    ...
    __properties: ClassVar[List[str]] = ["code", "csrf_token", "email", "method", "recovery_address", "recovery_confirm_address", "recovery_select_address", "screen", "transient_payload"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "recovery_address": obj.get("recovery_address"),
            "recovery_confirm_address": obj.get("recovery_confirm_address"),
            "recovery_select_address": obj.get("recovery_select_address"),

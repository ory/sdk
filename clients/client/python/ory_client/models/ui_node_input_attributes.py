...
class UiNodeInputAttributes(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["autocomplete", "disabled", "label", "maxlength", "name", "node_type", "onclick", "onclickTrigger", "onload", "onloadTrigger", "options", "pattern", "required", "type", "value"]
    ...
    def autocomplete_validate_enum(cls, value):
        ...
        if value not in set(['email', 'tel', 'url', 'current-password', 'new-password', 'one-time-code', 'username', 'username webauthn']):
            raise ValueError("must be one of enum values ('email', 'tel', 'url', 'current-password', 'new-password', 'one-time-code', 'username', 'username webauthn')")
        return value

    @field_validator('node_type')
    def node_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['input']):
            raise ValueError("must be one of enum values ('input')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "options": [UiNodeInputAttributesOption.from_dict(_item) for _item in obj["options"]] if obj.get("options") is not None else None,

...
class GetJWKSResponseJwks(BaseModel):
    """
    A JSON Web Key Set (RFC 7517). Contains a single field `keys`, an array of JWK objects.
    """ # noqa: E501
    keys: List[GetJWKSResponseJwksKeysInner] = Field(description="Array of JWK objects. Each key has at minimum `kty` (key type) and `kid` (key ID) plus key-type-specific material (e.g. `x`/`crv` for OKP/Ed25519, `n`/`e` for RSA).")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "keys": [GetJWKSResponseJwksKeysInner.from_dict(_item) for _item in obj["keys"]] if obj.get("keys") is not None else None
        })

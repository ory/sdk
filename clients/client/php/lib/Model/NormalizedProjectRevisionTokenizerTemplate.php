...
class NormalizedProjectRevisionTokenizerTemplate implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public const SUBJECT_SOURCE_ID = 'id';
    public const SUBJECT_SOURCE_EXTERNAL_ID = 'external_id';
    public const SUBJECT_SOURCE_UNKNOWN_DEFAULT_OPEN_API = 'unknown_default_open_api';
     ...
     * @return string[]
     */
    public function getSubjectSourceAllowableValues()
    {
        return [
            self::SUBJECT_SOURCE_ID,
            self::SUBJECT_SOURCE_EXTERNAL_ID,
            self::SUBJECT_SOURCE_UNKNOWN_DEFAULT_OPEN_API,
        ];
    }
    ...
    public function __construct(?array $data = null)
    {
        ...
        $this->setIfExists('subjectSource', $data ?? [], 'id');
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        $allowedValues = $this->getSubjectSourceAllowableValues();
        if (!is_null($this->container['subjectSource']) && !in_array($this->container['subjectSource'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value '%s' for 'subjectSource', must be one of '%s'",
                $this->container['subjectSource'],
                implode("', '", $allowedValues)
            );
        }

        if (!is_null($this->container['ttl']) && !preg_match("/^([0-9]+([.][0-9]+)?(ns|us|µs|ms|s|m|h))+$/", $this->container['ttl'])) {
            $invalidProperties[] = "invalid value for 'ttl', must be conform to the pattern /^([0-9]+([.][0-9]+)?(ns|us|µs|ms|s|m|h))+$/.";
        }
     ...
     * Sets jwksUrl
     *
     * @param string|null $jwksUrl JSON Web Key URL  The key set signs the tokens this template issues. It is write only and the value will not be returned in response to a read request. An update that leaves this field empty keeps the stored key set; supply a new one to rotate.
     *
     * @return self
     */
     ...
     * Sets subjectSource
     *
     * @param string|null $subjectSource Subject source for the tokenizer  Can be either id or external_id or empty
     *
     * @return self
     */
    public function setSubjectSource($subjectSource)
    {
        if (is_null($subjectSource)) {
            throw new \InvalidArgumentException('non-nullable subjectSource cannot be null');
        }
        $allowedValues = $this->getSubjectSourceAllowableValues();
        if (!in_array($subjectSource, $allowedValues, true)) {
            $subjectSource = self::SUBJECT_SOURCE_UNKNOWN_DEFAULT_OPEN_API;
        }
    ...
    public function setTtl($ttl)
    {
        ...
        if ((!preg_match("/^([0-9]+([.][0-9]+)?(ns|us|µs|ms|s|m|h))+$/", ObjectSerializer::toString($ttl)))) {
            throw new \InvalidArgumentException("invalid value for \$ttl when calling NormalizedProjectRevisionTokenizerTemplate., must conform to the pattern /^([0-9]+([.][0-9]+)?(ns|us|µs|ms|s|m|h))+$/.");
        }

...
defmodule Ory.Api.Frontend do
  ...
  - `opts` (keyword): Optional parameters
    ...
    - `:identity_schema` (String.t): An optional identity schema to use for the login flow.
  ...
  - `opts` (keyword): Optional parameters
    - `:return_to` (String.t): The URL to return the browser to after the flow was completed.
    - `:skip_settings` (String.t): Skip redirection to the settings UI after the recovery flow was completed. Instead, the user will be redirected to the URL specified in `return_to` query parameter or the default return URL if `return_to` is not set.
  ...
  - `connection` (Ory.Connection): Connection to server
  - `provider` (String.t): The SAML provider ID as configured in the Ory Kratos configuration.
  - `code` (String.t): The single-use authorization code issued by Ory Polis for the unsolicited SAML response.
  - `opts` (keyword): Optional parameters
  ...
  - `{:error, Tesla.Env.t}` on failure
  """
  @spec create_id_p_initiated_saml_browser_login_flow(Tesla.Env.client, String.t, String.t, keyword()) :: {:ok, nil} | {:ok, Ory.Model.ErrorGeneric.t} | {:error, Tesla.Env.t}
  def create_id_p_initiated_saml_browser_login_flow(connection, provider, code, _opts \\ []) do
    request =
      %{}
      |> method(:get)
      |> url("/self-service/methods/saml/idp-initiated/#{provider}")
  ...
  - `{:ok, Ory.Model.WebAuthnRelatedOrigins.t}` on success
  - `{:error, Tesla.Env.t}` on failure
  """
  @spec get_web_authn_related_origins(Tesla.Env.client, keyword()) :: {:ok, Ory.Model.WebAuthnRelatedOrigins.t} | {:error, Tesla.Env.t}
  def get_web_authn_related_origins(connection, _opts \\ []) do
    request =
      %{}
      |> method(:get)
    ...
    |> Connection.request(request)
    |> evaluate_response([
      {200, Ory.Model.WebAuthnRelatedOrigins}
    ])
  end

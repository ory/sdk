...
export function instanceOfQuotaUsage(value: object): boolean {
    if (!('additional_price' in value)) return false;
    if (!('can_use_more' in value)) return false;
    if (!('feature' in value)) return false;
    if (!('feature_available' in value)) return false;
    if (!('included' in value)) return false;

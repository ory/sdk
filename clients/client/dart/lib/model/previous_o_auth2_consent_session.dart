...
class PreviousOAuth2ConsentSession {
  /// Returns a new [PreviousOAuth2ConsentSession] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is PreviousOAuth2ConsentSession &&
  ...
  @override
  String toString() => 'PreviousOAuth2ConsentSession[consentRequest=$consentRequest, grantAccessTokenAudience=$grantAccessTokenAudience, grantScope=$grantScope, handledAt=$handledAt, remember=$remember, rememberFor=$rememberFor, session=$session]';
  ...
  /// Returns a new [PreviousOAuth2ConsentSession] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static PreviousOAuth2ConsentSession? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PreviousOAuth2ConsentSession[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PreviousOAuth2ConsentSession[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PreviousOAuth2ConsentSession>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <PreviousOAuth2ConsentSession>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PreviousOAuth2ConsentSession.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PreviousOAuth2ConsentSession> mapFromJson(dynamic json) {
    final map = <String, PreviousOAuth2ConsentSession>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PreviousOAuth2ConsentSession.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of PreviousOAuth2ConsentSession-objects as value to a dart map
  static Map<String, List<PreviousOAuth2ConsentSession>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PreviousOAuth2ConsentSession>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PreviousOAuth2ConsentSession.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }

...
public class IdentityWithCredentialsOidcConfig {
  ...
  private List<IdentityWithCredentialsOidcConfigProvider> providers = null;
  ...
  * @throws IOException if the JSON Object is invalid with respect to IdentityWithCredentialsOidcConfig
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!IdentityWithCredentialsOidcConfig.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!IdentityWithCredentialsOidcConfig.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `IdentityWithCredentialsOidcConfig` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      // validate the optional field `config`
      if (jsonObj.get("config") != null && !jsonObj.get("config").isJsonNull()) {
        IdentityWithCredentialsPasswordConfig.validateJsonObject(jsonObj.getAsJsonObject("config"));
      }
      if (jsonObj.get("providers") != null && !jsonObj.get("providers").isJsonNull()) {
        ...
        if (jsonArrayproviders != null) {
          ...
          for (int i = 0; i < jsonArrayproviders.size(); i++) {
            IdentityWithCredentialsOidcConfigProvider.validateJsonObject(jsonArrayproviders.get(i).getAsJsonObject());
          };
        }
      }
  }

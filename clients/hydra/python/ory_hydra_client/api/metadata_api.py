...
class MetadataApi(object):
    ...
    def __init__(self, api_client=None):
        ...
        self.get_version_endpoint = _Endpoint(
            settings={
                'response_type': (GetVersion200Response,),
                'auth': [],
        ...
        self.is_ready_endpoint = _Endpoint(
            settings={
                'response_type': (IsReady200Response,),
                'auth': [],
    ...
    def is_ready(
        ...
        **kwargs
        ...
        This endpoint returns a HTTP 200 status code when Ory Hydra is up running and the environment dependencies (e.g. the database) are responsive as well.  If the service supports TLS Edge Termination, this endpoint does not require the `X-Forwarded-Proto` header to be set.  Be aware that if you are running multiple nodes of Ory Hydra, the health status will never refer to the cluster state, only to a single instance.  # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True

...
class RecoveryFlowState(ModelSimple):
    ...
    def __init__(self, *args, **kwargs):
        """RecoveryFlowState - a model defined in OpenAPI

        Note that value can be passed either in args or in kwargs, but not in both.

        Args:
            args[0] (str): The state represents the state of the recovery flow.  choose_method: ask the user to choose a method (e.g. recover account via email) sent_email: the email has been sent to the user passed_challenge: the request was successful and the recovery challenge was passed.., must be one of ["choose_method", "sent_email", "passed_challenge", ]  # noqa: E501

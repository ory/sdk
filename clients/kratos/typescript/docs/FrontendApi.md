...
let initCode: string; //The part of the code return when initializing the flow. (default to undefined)
let returnToCode: string; //The part of the code returned by the return_to URL. (default to undefined)

const { status, data } = await apiInstance.exchangeSessionToken(
    initCode,
    returnToCode
);
```
...
let id: string; //Error is the error\'s ID (default to undefined)

const { status, data } = await apiInstance.getFlowError(
    id
);
```
...
let id: string; //The Login Flow ID  The value for this parameter comes from `flow` URL Query parameter sent to your application (e.g. `/login?flow=abcde`). (default to undefined)
let cookie: string; //HTTP Cookies  When using the SDK in a browser app, on the server side you must include the HTTP Cookie Header sent by the client to your server here. This ensures that CSRF and session cookies are respected. (optional) (default to undefined)

const { status, data } = await apiInstance.getLoginFlow(
    id,
    cookie
);
```
...
let id: string; //The Flow ID  The value for this parameter comes from `request` URL Query parameter sent to your application (e.g. `/recovery?flow=abcde`). (default to undefined)
let cookie: string; //HTTP Cookies  When using the SDK in a browser app, on the server side you must include the HTTP Cookie Header sent by the client to your server here. This ensures that CSRF and session cookies are respected. (optional) (default to undefined)

const { status, data } = await apiInstance.getRecoveryFlow(
    id,
    cookie
);
```
...
let id: string; //The Registration Flow ID  The value for this parameter comes from `flow` URL Query parameter sent to your application (e.g. `/registration?flow=abcde`). (default to undefined)
let cookie: string; //HTTP Cookies  When using the SDK in a browser app, on the server side you must include the HTTP Cookie Header sent by the client to your server here. This ensures that CSRF and session cookies are respected. (optional) (default to undefined)

const { status, data } = await apiInstance.getRegistrationFlow(
    id,
    cookie
);
```
...
let id: string; //ID is the Settings Flow ID  The value for this parameter comes from `flow` URL Query parameter sent to your application (e.g. `/settings?flow=abcde`). (default to undefined)
let xSessionToken: string; //The Session Token  When using the SDK in an app without a browser, please include the session token here. (optional) (default to undefined)
let cookie: string; //HTTP Cookies  When using the SDK in a browser app, on the server side you must include the HTTP Cookie Header sent by the client to your server here. This ensures that CSRF and session cookies are respected. (optional) (default to undefined)

const { status, data } = await apiInstance.getSettingsFlow(
    id,
    xSessionToken,
    cookie
);
...
let id: string; //The Flow ID  The value for this parameter comes from `request` URL Query parameter sent to your application (e.g. `/verification?flow=abcde`). (default to undefined)
let cookie: string; //HTTP Cookies  When using the SDK on the server side you must include the HTTP Cookie Header originally sent to your HTTP handler here. (optional) (default to undefined)

const { status, data } = await apiInstance.getVerificationFlow(
    id,
    cookie
);
```
...
const { status, data } = await apiInstance.getWebAuthnJavaScript();
```
...
const { status, data } = await apiInstance.listMySessions(
    perPage,
    page,
    pageSize,
    pageToken,
...
let performNativeLogoutBody: PerformNativeLogoutBody; //

const { status, data } = await apiInstance.performNativeLogout(
    performNativeLogoutBody
);
```
...
let xSessionToken: string; //Set the Session Token when calling from non-browser clients. A session token has a format of `MP2YWEMeM8MxjkGKpH4dqOQ4Q4DlSPaj`. (optional) (default to undefined)
let cookie: string; //Set the Cookie Header. This is especially useful when calling this endpoint from a server-side application. In that scenario you must include the HTTP Cookie Header which originally was included in the request to your server. An example of a session in the HTTP Cookie Header is: `ory_kratos_session=a19iOVAbdzdgl70Rq1QZmrKmcjDtdsviCTZx7m9a9yHIUS8Wa9T7hvqyGTsLHi6Qifn2WUfpAKx9DWp0SJGleIn9vh2YF4A16id93kXFTgIgmwIOvbVAScyrx7yVl6bPZnCx27ec4WQDtaTewC1CpgudeDV2jQQnSaCP6ny3xa8qLH-QUgYqdQuoA_LF1phxgRCUfIrCLQOkolX5nv3ze_f==`.  It is ok if more than one cookie are included here as all other cookies will be ignored. (optional) (default to undefined)
let tokenizeAs: string; //Returns the session additionally as a token (such as a JWT)  The value of this parameter has to be a valid, configured Ory Session token template. For more information head over to [the documentation](http://ory.sh/docs/identities/session-to-jwt-cors). (optional) (default to undefined)

const { status, data } = await apiInstance.toSession(
    xSessionToken,
    cookie,
    tokenizeAs
);
...
let updateFedcmFlowBody: UpdateFedcmFlowBody; //

const { status, data } = await apiInstance.updateFedcmFlow(
    updateFedcmFlowBody
);
```
...
let flow: string; //The Login Flow ID  The value for this parameter comes from `flow` URL Query parameter sent to your application (e.g. `/login?flow=abcde`). (default to undefined)
let updateLoginFlowBody: UpdateLoginFlowBody; //
let xSessionToken: string; //The Session Token of the Identity performing the settings flow. (optional) (default to undefined)
let cookie: string; //HTTP Cookies  When using the SDK in a browser app, on the server side you must include the HTTP Cookie Header sent by the client to your server here. This ensures that CSRF and session cookies are respected. (optional) (default to undefined)

const { status, data } = await apiInstance.updateLoginFlow(
    flow,
    updateLoginFlowBody,
    xSessionToken,
    cookie
...
let token: string; //A Valid Logout Token  If you do not have a logout token because you only have a session cookie, call `/self-service/logout/browser` to generate a URL for this endpoint. (optional) (default to undefined)
let returnTo: string; //The URL to return to after the logout was completed. (optional) (default to undefined)
let cookie: string; //HTTP Cookies  When using the SDK in a browser app, on the server side you must include the HTTP Cookie Header sent by the client to your server here. This ensures that CSRF and session cookies are respected. (optional) (default to undefined)

const { status, data } = await apiInstance.updateLogoutFlow(
    token,
    returnTo,
    cookie
);
...
let flow: string; //The Recovery Flow ID  The value for this parameter comes from `flow` URL Query parameter sent to your application (e.g. `/recovery?flow=abcde`). (default to undefined)
let updateRecoveryFlowBody: UpdateRecoveryFlowBody; //
let token: string; //Recovery Token  The recovery token which completes the recovery request. If the token is invalid (e.g. expired) an error will be shown to the end-user.  This parameter is usually set in a link and not used by any direct API call. (optional) (default to undefined)
let cookie: string; //HTTP Cookies  When using the SDK in a browser app, on the server side you must include the HTTP Cookie Header sent by the client to your server here. This ensures that CSRF and session cookies are respected. (optional) (default to undefined)

const { status, data } = await apiInstance.updateRecoveryFlow(
    flow,
    updateRecoveryFlowBody,
    token,
    cookie
...
let flow: string; //The Registration Flow ID  The value for this parameter comes from `flow` URL Query parameter sent to your application (e.g. `/registration?flow=abcde`). (default to undefined)
let updateRegistrationFlowBody: UpdateRegistrationFlowBody; //
let cookie: string; //HTTP Cookies  When using the SDK in a browser app, on the server side you must include the HTTP Cookie Header sent by the client to your server here. This ensures that CSRF and session cookies are respected. (optional) (default to undefined)

const { status, data } = await apiInstance.updateRegistrationFlow(
    flow,
    updateRegistrationFlowBody,
    cookie
);
...
let flow: string; //The Settings Flow ID  The value for this parameter comes from `flow` URL Query parameter sent to your application (e.g. `/settings?flow=abcde`). (default to undefined)
let updateSettingsFlowBody: UpdateSettingsFlowBody; //
let xSessionToken: string; //The Session Token of the Identity performing the settings flow. (optional) (default to undefined)
let cookie: string; //HTTP Cookies  When using the SDK in a browser app, on the server side you must include the HTTP Cookie Header sent by the client to your server here. This ensures that CSRF and session cookies are respected. (optional) (default to undefined)

const { status, data } = await apiInstance.updateSettingsFlow(
    flow,
    updateSettingsFlowBody,
    xSessionToken,
    cookie
...
let flow: string; //The Verification Flow ID  The value for this parameter comes from `flow` URL Query parameter sent to your application (e.g. `/verification?flow=abcde`). (default to undefined)
let updateVerificationFlowBody: UpdateVerificationFlowBody; //
let token: string; //Verification Token  The verification token which completes the verification request. If the token is invalid (e.g. expired) an error will be shown to the end-user.  This parameter is usually set in a link and not used by any direct API call. (optional) (default to undefined)
let cookie: string; //HTTP Cookies  When using the SDK in a browser app, on the server side you must include the HTTP Cookie Header sent by the client to your server here. This ensures that CSRF and session cookies are respected. (optional) (default to undefined)

const { status, data } = await apiInstance.updateVerificationFlow(
    flow,
    updateVerificationFlowBody,
    token,
    cookie

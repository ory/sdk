...
```php
createBrowserLoginFlow($refresh, $aal, $returnTo, $cookie, $loginChallenge, $organization): \Ory\Kratos\Client\Model\LoginFlow
```
...
try {
    $result = $apiInstance->createBrowserLoginFlow($refresh, $aal, $returnTo, $cookie, $loginChallenge, $organization);
    print_r($result);
} catch (Exception $e) {
...
```php
createNativeLoginFlow($refresh, $aal, $xSessionToken, $returnSessionTokenExchangeCode, $returnTo, $via): \Ory\Kratos\Client\Model\LoginFlow
```
...
```php
createNativeVerificationFlow(): \Ory\Kratos\Client\Model\VerificationFlow
```

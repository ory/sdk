...
```php
createBrowserLoginFlow($refresh, $aal, $returnTo, $cookie, $loginChallenge, $organization, $via): \Ory\Kratos\Client\Model\LoginFlow
```
...
```php
createBrowserRegistrationFlow($returnTo, $loginChallenge, $afterVerificationReturnTo, $organization): \Ory\Kratos\Client\Model\RegistrationFlow
```
...
```php
createNativeLoginFlow($refresh, $aal, $xSessionToken, $returnSessionTokenExchangeCode, $returnTo, $organization, $via): \Ory\Kratos\Client\Model\LoginFlow
```
...
```php
createNativeRegistrationFlow($returnSessionTokenExchangeCode, $returnTo, $organization): \Ory\Kratos\Client\Model\RegistrationFlow
```

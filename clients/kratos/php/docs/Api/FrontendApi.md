...
```php
createBrowserLoginFlow($refresh, $aal, $returnTo, $cookie, $loginChallenge): \Ory\Kratos\Client\Model\LoginFlow
```
...
```php
createBrowserRegistrationFlow($returnTo, $loginChallenge, $afterVerificationReturnTo): \Ory\Kratos\Client\Model\RegistrationFlow
```
...
```php
listMySessions($perPage, $page, $xSessionToken, $cookie): \Ory\Kratos\Client\Model\Session[]
```

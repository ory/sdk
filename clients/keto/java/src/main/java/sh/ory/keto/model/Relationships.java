...
public class Relationships {
  ...
  private List<Relationship> relationTuples = null;
  ...
  * @throws IOException if the JSON Object is invalid with respect to Relationships
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!Relationships.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!Relationships.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Relationships` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      if (jsonObj.get("relation_tuples") != null && !jsonObj.get("relation_tuples").isJsonNull()) {
        ...
        if (jsonArrayrelationTuples != null) {
          ...
          for (int i = 0; i < jsonArrayrelationTuples.size(); i++) {
            Relationship.validateJsonObject(jsonArrayrelationTuples.get(i).getAsJsonObject());
          };
        }
      }
  }
